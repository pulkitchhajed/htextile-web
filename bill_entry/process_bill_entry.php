		...
		if($bill_entry_id_check>-1){
			...
			if($_FILES['bill_upload']['name']!="") { // only if files are selected -- to pevent blank entry in table
				...
				if($file_size_bill_upload > 10485760){  // 2097152  
					...
					$upload_result = supabase_upload_file($file_tmp_bill_upload, "bills/" . $file_name_bill_upload, $file_type_bill_upload);
					if (!$upload_result['success']) { error_log("Supabase upload failed: " . $upload_result['error']); }
					$bill_upload="bills/".$file_name_bill_upload;
			}	
				$supporting_doc="";
			if($_FILES['supporting_doc']['name']!="") { // only if files are selected -- to pevent blank entry in table
				...
				if($file_size_supporting_doc > 10485760){  // 2097152
					...
					$upload_result = supabase_upload_file($file_tmp_supporting_doc, "bills/support_" . $file_name_supporting_doc, $file_type_supporting_doc);
					if (!$upload_result['success']) { error_log("Supabase upload failed: " . $upload_result['error']); }
					$supporting_doc="bills/support_".$file_name_supporting_doc;
			}				
