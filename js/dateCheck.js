function validatedate(inputField) {
    ...
    if (inputField.value.match(dateformat)) {
        ...
        opera1 = opera1.length;
        opera2 = opera2.length;
...
function validate_currentdate_format(field) {
    //alert(isDate(field.value, "-"));
    if (field.value != "") {
        if (!isDate(field.value, "-")) {
            alert('Date Invalid');
            field.focus();
        } else if (!isCurrentDate(field)) {
            alert('Can not be Future Date ');
            field.focus();
...
//Value parameter - required. All other parameters are optional.
function isCurrentDate(value, sepVal, dayIdx, monthIdx, yearIdx) {
    try {
        //Change the below values to determine which format of date you wish to check. It is set to dd/mm/yyyy by default.
        var DayIndex = dayIdx !== undefined ? dayIdx : 0;
        var MonthIndex = monthIdx !== undefined ? monthIdx : 1;
        ...
        if (OK) {
            ...
            testCurrDate = new Date(currDate().getFullYear, currDate.getMonth(), currDate.getDate());
            alert(currDate.getDate());
            if (inputDate > testCurrDate) {
                OK = false;
