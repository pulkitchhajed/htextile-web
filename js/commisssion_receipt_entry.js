...
// New Renamed  payVouCalculateAmount //
function arrayCalculateAmount(i_total_ele, i_array, i_amt) {
...
//    logMessageOne("Inside payVouCalculateAmount");
//    logMessageTwo("Inside payVouCalculateAmount");
...
function logMessageOne (i_msg){
        // Log Message for Java Script Debugging

        
        document.getElementById('log_message_one').disabled=false;
        document.getElementById('log_message_one').value=i_msg;
        document.getElementById('log_message_one').disabled=true;
        
    
}

function logMessageTwo (i_msg){
    // Log Message for Java Script Debugging
    
    document.getElementById('log_message_two').disabled=false;
    document.getElementById('log_message_two').value=i_msg;
    document.getElementById('log_message_two').disabled=true;
    

}

//New //
function calculateDeductionAmt(){

    updateDeductionAndRecpt();
    arrayCalculateAmount('total_amount_received', 'bill_received_amt[]', 0);        
    arrayCalculateAmount('total_deduction', 'deduction_amt[]', 0);    
    calculateBillBalanceAmount();


}
//New //
function calculateRecdAmt(){
...
//New //
function receiptBillPayOnChange(){
...
//New //
function receiptPartFullOnChange(){
    // if not selected please remove the value  
//    logMessageOne("Inside receiptFull");
...
//New //
function updateDeductionAndRecpt(){
    // receipt_part_full

    //deduction_amt
    //bill_received_amt    
    logMessageOne("Inside updateDeductionAndRecpt");
    var l_receipt_part_full_array = document.getElementsByName('receipt_part_full[]');    

    var l_deduction_amt_array = document.getElementsByName('deduction_amt[]');
    var l_bill_received_amt_array = document.getElementsByName('bill_received_amt[]');

    l_size = l_receipt_part_full_array.length;
    for (a = 0; a < l_size; a++) {

        l_receipt_part_full=l_receipt_part_full_array[a].value;        

        if(l_receipt_part_full=="" || l_receipt_part_full=="Select"){

            l_deduction_amt_array[a].value=processBlanktoZero(""); 
            l_bill_received_amt_array[a].value=processBlanktoZero("");             
            // l_receipt_part_full_array[a].focus();
            // return false;
        }
    }
}
...
// New //
function calculateBillBalanceAmount(){
    // Need to calculate Balance As per Receipt Wise Selection 
    ...
    logMessageOne("Inside calculateBillBalanceAmount");
    ...
    var l_receipt_bill_pay_array = document.getElementsByName('receipt_bill_pay[]');
    var l_receipt_part_full_array = document.getElementsByName('receipt_part_full[]');
    ...
    logMessageTwo(" All Var Set calculateBillBalanceAmount");

    l_size = l_commission_amt_bill_array.length;
    for (a = 0; a < l_size; a++) {

        l_commission_mode_array[a].disabled = false;
        l_commission_mode=l_commission_mode_array[a].value
        l_commission_mode_array[a].disabled = true;
        

        l_commission_amt_bill_array[a].disabled = false;
        l_commission_amt_bill = processBlanktoZero(l_commission_amt_bill_array[a].value).toFixed(2);
        l_commission_amt_bill_array[a].disabled = true;

        l_commission_amt_pay_array[a].disabled = false;
        l_commission_amt_pay = processBlanktoZero(l_commission_amt_pay_array[a].value).toFixed(2);
        l_commission_amt_pay_array[a].disabled = true;

        l_adj_amt_array[a].disabled = false;
        l_adj_amt = processBlanktoZero(l_adj_amt_array[a].value).toFixed(2);
        l_adj_amt_array[a].disabled = true;

        l_adj_dis_array[a].disabled = false;
        l_adj_dis = processBlanktoZero(l_adj_dis_array[a].value).toFixed(2);
        l_adj_dis_array[a].disabled = true;

        l_receipt_bill_pay=l_receipt_bill_pay_array[a].value;
        l_receipt_part_full=l_receipt_part_full_array[a].value;

        l_deduction_amt = processBlanktoZero(l_deduction_amt_array[a].value).toFixed(2);
        l_bill_received_amt = processBlanktoZero(l_bill_received_amt_array[a].value).toFixed(2);
        
        l_bill_bal=processBlanktoZero(l_bill_bal_amt_array[a].value).toFixed(2);

        if(l_receipt_bill_pay=="Bill Wise"){
            l_bill_bal=l_commission_amt_bill-l_adj_amt-l_adj_dis-l_deduction_amt-l_bill_received_amt;
        } else if(l_receipt_bill_pay=="Payment Wise"){
            l_bill_bal=l_commission_amt_pay-l_adj_amt-l_adj_dis-l_deduction_amt-l_bill_received_amt;
        }else if (l_commission_mode=="Bill Wise"){
            l_bill_bal=l_commission_amt_bill-l_adj_amt-l_adj_dis-l_deduction_amt-l_bill_received_amt;
        }else if (l_commission_mode=="Payment Wise"){
            l_bill_bal=l_commission_amt_pay-l_adj_amt-l_adj_dis-l_deduction_amt-l_bill_received_amt;
        }else {
...
function agentChange() {
    l_agent_last = document.getElementById('agent_last_value').value;
    l_agent_selected = document.getElementById('agent_account_code').value;
    if (l_agent_last != "") {
        if (l_agent_last != l_agent_selected) {
            alert("You have Changed Agent Please Click Next to continue");
            document.getElementById('agent_tag').value = "FALSE";
        }
        if (l_agent_last == l_agent_selected) {
            document.getElementById('agent_tag').value = "";
        }
    }
}
...
function receiptModeChange(){
    l_receipt_mode_last = document.getElementById('receipt_mode_last_value').value;
    l_receipt_mode = document.getElementById('receipt_mode').value;
    if (l_receipt_mode_last != "") {
        if (l_receipt_mode_last != l_receipt_mode) {
            alert("You have Changed Receipt Mode Please Click Next to continue");
            document.getElementById('receipt_mode_tag').value = "FALSE";
        }
        if (l_receipt_mode_last == l_receipt_mode) {
            document.getElementById('receipt_mode_tag').value = "";
        }
    }        
...
function receiptTypeChange(){
    l_receipt_type_last = document.getElementById('receipt_type_last_value').value;
    l_receipt_type = document.getElementById('receipt_type').value;
    if (l_receipt_type_last != "") {
        if (l_receipt_type_last != l_receipt_type) {
            alert("You have Changed Receipt Type Please Click Next to continue");
            document.getElementById('receipt_type_tag').value = "FALSE";
        }
        if (l_receipt_type_last == l_receipt_type) {
            document.getElementById('receipt_type_tag').value = "";
        }
    }  
...
function recptAmountOnChange(){
    l_receipt_amount_last = document.getElementById('receipt_amount_last_value').value;
    l_receipt_amount = document.getElementById('receipt_amount').value;
    if (l_receipt_amount_last != "") {
        if (l_receipt_amount_last != l_receipt_amount) {
                alert("You have Changed Receipt Amount Please Click Next to continue");
                document.getElementById('receipt_amount_tag').value = "FALSE";
        }
        if (l_receipt_amount_last == l_receipt_amount) {
            document.getElementById('receipt_amount_tag').value = "";
        }
    }  
}
...
function deductionAmountOnChange(){
    l_deduction_amount_last = document.getElementById('deduction_amount_last_value').value;
    l_deduction_amount = document.getElementById('deduction_amount').value;
    if (l_deduction_amount_last != "") {
        if (l_deduction_amount_last != l_deduction_amount) {
                alert("You have Changed Deduction Amount Please Click Next to continue");
                document.getElementById('deduction_amount_tag').value = "FALSE";
        }
        if (l_deduction_amount_last == l_deduction_amount) {
            document.getElementById('deduction_amount_tag').value = "";
        }
    } 
}
...
//New - updated calculateDifference //
function calculateDifference() {
    //total_amount
    //total_discount
    //total_goods_return
    val_total_amt = processBlanktoZero(document.getElementById('receipt_amount').value);
    val_total_dis = processBlanktoZero(document.getElementById('deduction_amount').value);
    ...
    val_total_dis_rec = processBlanktoZero(document.getElementById('total_deduction').value);
    document.getElementById('total_deduction').disabled = true;
    ...
    document.getElementById('discount_difference').value = val_dis_rec_diff;
    document.getElementById('discount_difference').disabled = true;
...
function checkAllDifference(){
	...
	l_dis_rec_diff_val=document.getElementById("discount_difference").value;
	

	if(l_amt_rec_diff_val!=0 || l_dis_rec_diff_val !=0   ){
		alert("All Difference should be 0 Please Adjust the Amount Properly");
		document.getElementsByName('deduction_amt[]')[0].focus();
		return false;
	}
	return true;
...
function checkBillPaymentTypeSelected()
{
	...
	var l_bill_payment_type_array=document.getElementsByName('receipt_part_full[]');
	
	var l_bill_deduction_amt_array=document.getElementsByName('deduction_amt[]');
    var l_bill_received_amt_array=document.getElementsByName('bill_received_amt[]');
	var l_bill_bal_amt_array=document.getElementsByName('bill_bal_amt[]');
	...
	for(a=0;a<l_size;a++){
		...
		if(l_bill_type_val=="Part" &&  l_bill_ded_amt_val==0 &&  l_bill_rec_amt_val==0 )
		{
			alert('Please enter Value for Deduction or Amount Received');
			l_bill_received_amt_array[a].focus();
			return false;
		}

		// If Bill Payment Type is not selected and Value is available for Amout REceived or Discount or Goods Return
		if(l_bill_type_val=="" && ( l_bill_ded_amt_val>0 ||  l_bill_rec_amt_val>0)){
...
function balanceLessThenZero() {
    //alert("Check Balance");
    // Bill Window  Bal Amount Less Then Zero 
    var l_bill_bal_amt_array = document.getElementsByName('bill_bal_amt[]');
    var l_bill_received_amt_array = document.getElementsByName('bill_received_amt[]');
    // var l_bill_payment_type_array = document.getElementsByName('receipt_bill_pay[]');
...
// Old - Updated -- New Field Name and different combination//
function checkMinOneBillHasValue() {
    //alert('Hello');
    
    var l_receipt_type_selected = document.getElementById('receipt_type').value;


        // Check Rec Amount and Discount 
        if (l_receipt_type_selected == "Receipt") {
        ...
        } else if (l_receipt_type_selected == "TDS" || l_receipt_type_selected == "Misc Less") {
            var l_total_discount_received_val = processBlanktoZero(document.getElementById('total_deduction').value);
            if (l_total_discount_received_val > 0) {
                var l_total_amount_received_val = processBlanktoZero(document.getElementById('total_amount_received').value);                
                if (l_total_amount_received_val > 0) {
                    alert("Please Remove Received Amount in Bill Section ");
                    var l_bill_received_amt_array = document.getElementsByName('bill_received_amt[]');
                    l_bill_received_amt_array[0].focus();
                    return false;
                }else{
                ...
                alert("Please Enter Deduction in Bill Section ");
                var l_bill_discount_amt_array = document.getElementsByName('deduction_amt[]');
                l_bill_discount_amt_array[0].focus();
                return false;
            }

        } else if (l_receipt_type_selected == "Advance Receipt") {
            return true;
        } else {
            // This is very unrealistic Scenario this will occur only when some of the predefined Item in Payment Type will change.
            alert('Please Enter the Required Details in Bill Section');
