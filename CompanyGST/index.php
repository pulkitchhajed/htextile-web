...
$sql="select * from txt_company where delete_tag='FALSE' and firm_type='Buyer' order by company_id ASC";
$result=mysqli_query($con,$sql);
...
<table class="tbl_border">
	...
	while($rs=mysqli_fetch_array($result)) 
	{
		...
		if (($arr[$a]=="Supplier") && (($_SESSION['ROLEID']=='admin' || $_SESSION['ROLEID']=='master' ) ) ){
			...
			echo "<td>".$rs['comm_stat_pref']."</td>";
		}else {
			// echo "<td>".$rs['sms_number']."</td>";
			// echo "<td>".$rs['whatsapp_number']."</td>";

			echo "<td>".$rs['bank_name']."</td>";
			echo "<td>".$rs['bank_account_number']."</td>";
		}
