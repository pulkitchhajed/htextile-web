...
$s_sql="SELECT * FROM txt_company WHERE Firm_type='Supplier' AND delete_tag='FALSE' order by company_id ASC";
...
/*
$arr_sup_rep_type=array("Value"=>"Key");
$sup_arr=array($s_result['comm_stat_pref']=>$s_result['company_id']);
$arr_sup_rep_type=array_merge($arr_sup_rep_type,$sup_arr);
...
while($s_rs=mysqli_fetch_array($s_result))
{
  ...
  echo "{";
  echo 'company_id:';
  echo $s_rs['company_id'];
  echo ",";

  echo "comm_stat_pref:'";
  echo $s_rs['comm_stat_pref'];
  echo "'";
...
{ company_id :0,comm_stat_pref:'hello'}
...
<script>
  ...
  function suppOnChange(){
// // Find the object where id is 2
// const result = array.find(item => item.id === 2);

// supplier_account_code

//alert ("inside supp on change ");



var supp_selected_id=document.getElementById('supplier_account_code').value;

//alert (supp_selected_id);


const stat_type=supp_array.find(comp => comp.company_id == supp_selected_id);

//alert ("Hello");

//alert (stat_type.comm_stat_pref);

document.getElementById('comm_report_type').value=stat_type.comm_stat_pref;
            ...
            <div class="content_padding">
              ...
              <table class="tbl_border">
                <tr>
                  ...
                  <td>
                    <select onchange='suppOnChange()' name="supplier_account_code" id="supplier_account_code">
...
*/
                  ...
                  <td>
                      <select onchange='downloadLock()' name="comm_report_type" id="comm_report_type">
                            <?php
                                $arr=array('BillWise','PaymentWise');
