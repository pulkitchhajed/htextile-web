...
ON bill_supp_company_id=pay_supp_company_id
	...
	*/
	$sql_comm_bill=" SELECT bill_supplier_name,
							...
							total_payment AS total_bill_payment,
							pay_supp_company_id,
							supp_comm,comm_stat_pref,agent_id,city
					FROM 
					(SELECT supp_firm_name AS bill_supplier_name , 
							supp_group_name AS bill_supp_group_name,
							supp_company_id AS bill_supp_company_id,
							comm_stat_pref AS comm_stat_pref, 
							sum(bill_amount ) AS total_bill_amount,
							AVG(comm_per) AS supp_comm ,
							agent_id,city
					FROM txt_bill_entry ,( SELECT supp.company_id AS supp_company_id,
												  ...
												  supp.commission_percentage AS comm_per,
												  supp.comm_stat_pref AS comm_stat_pref,
												  grp.group_name AS supp_group_name
											FROM txt_company AS supp ,txt_group_master AS grp
											WHERE supp.delete_tag='FALSE' 
											AND firm_type='Supplier'
...
*/					
	...
	<tr><th colspan='14'><h3>Commission Summary Report (Bill Wise)</h3></th></tr>
	<tr><th colspan='14'><b><?php echo " From ".$start_date." To ".$end_date ?></b></th></tr>
	...
	echo "<tr>";
	echo "<td align='right' colspan='6'> Total </td>";
...
if($comm_report_type=="Payment Wise"){
	...
	*/
	...
	$sql_comm_pay=" SELECT sum(discount_amount) AS total_discount, 
						  ...
						  AVG(comm_per) AS supp_comm,
						  supp_firm_name  AS pay_supplier_name,
						  supp_group_name AS pay_group_name ,comm_stat_pref,agent_id ,city
					FROM txt_payment_entry_main ,( SELECT supp.company_id AS supp_company_id,
														  supp.firm_name AS supp_firm_name ,
														  supp.commission_percentage AS comm_per ,
														  supp.comm_stat_pref AS comm_stat_pref,
...
*/
	...
	$count=0;
?>
<tr><th colspan='13'><h3>Commission Summary Report (Payment Wise)</h3></th></tr>
...
<?php 
	...
	$sno=0;
	$page_total_comm_amt=0;
	$page_total_payment=0;
	$page_total_comm_less_gst=0;
...
*/
		...
		ORDER BY pay_supplier_name
			...
			*/
			...
			supp_comm  AS supp_comm_pay,
			bill_supplier_name AS pay_supplier_name,
			bill_supp_group_name As pay_group_name ,agent_id,city,comm_stat_pref
		FROM 
		(SELECT supp_firm_name AS bill_supplier_name , 
				supp_group_name AS bill_supp_group_name,
				supp_company_id AS bill_supp_company_id,
				sum(bill_amount ) AS total_bill_amount,
				AVG(comm_per) AS supp_comm ,comm_stat_pref,agent_id,city
		FROM txt_bill_entry ,( SELECT supp.company_id AS supp_company_id,
									supp.firm_name AS supp_firm_name ,
									supp.commission_percentage AS comm_per,
									supp.comm_stat_pref AS comm_stat_pref,
...
if($end_date!=''){
	$sql_comm_bill_both.=" AND bill_date<='$rep_end_date' ";
		...
		*/			  
		...
		AVG(comm_per) AS supp_comm_pay,
		supp_firm_name  AS pay_supplier_name,
		supp_group_name AS pay_group_name ,agent_id,city,comm_stat_pref
		FROM txt_payment_entry_main ,( SELECT supp.company_id AS supp_company_id,
										supp.firm_name AS supp_firm_name ,
										supp.commission_percentage AS comm_per,
										supp.comm_stat_pref AS comm_stat_pref,
...
*/
		...
		$count=0;
?>
		<tr><th colspan='17'><h3>Commission Summary Report (Bill Wise & Payment Wise)</h3></th></tr>
		<tr><th colspan='17'><b><?php echo " From ".$start_date." To ".$end_date ?></b></th></tr>
...
<?php
$page_total_comm_amt_pay_less_gst=$page_total_gr=0;
$page_total_comm_amt_ill_less_gst=$page_total_bill_amt=0;
$page_total_discount=$page_total_net_bill_amt=0;
$page_total_comm_amt_bill=$page_total_comm_amt_bill_less_gst=0;
$page_total_payment_pay=$page_total_comm_amt_pay=0;
$sno=0;
while($rs= mysqli_fetch_array($result)){
