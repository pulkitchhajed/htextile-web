            ...
            <table width="100%"><tr><td valign="bottom" width="335" ><h3>Payment Entry :</h3> <h4>Dispalyed in order of last updated</h4></td>
             <td align="center" >
                    <?php
								...
								?></td>
            	<td align="right"> 
                <button onclick="location.href='index.php'">Refresh</button> &nbsp; &nbsp; 
                <button onClick="location.href='commission_add_receipt_entry.php'">Add Commission Receipt</button></td>
            </tr>
            </table>
            </div>
...
<table class="tbl_border">
	...
	<tr>
      ...
      <th valign="top" >View</th>      
        <th valign="top" >Receipt Id</th>
        
        <th valign="top" >Receipt Date</th>
        <th valign="top" >Receipt Type</th>        
        <th valign="top" >Receipt Mode</th>                
        <th valign="top" >Agent Name</th>
        <th valign="top" >Supplier Name</th>
        <th valign="top" >Receipt Amount</th>
...
$sql_pay="select * from txt_commission_receipt_entry_main 
        ...
        order by last_update_date desc,receipt_date DESC, receipt_entry_id desc LIMIT 0,50";
        
$result=mysqli_query($con,$sql_pay);
$count=0;
while($rs=mysqli_fetch_array($result))
{
	$receipt_entry_id=$rs[0];
	echo "<tr>";
  echo "<td>".++$count."</td>";
  echo "<td><a href='commission_edit_receipt_entry.php?receipt_entry_id=$receipt_entry_id'><img src='../images/noun_project_edit_1.png' height='16' width='16' border='0' title='Edit' /></a></td>";
  echo "<td><a href='commission_view_receipt_entry.php?receipt_entry_id=$receipt_entry_id'><img src='../images/noun_project_preview_1.png' height='16' width='16' border='0' title='View' /></a></td>";
  echo "<td align='left' >".$rs['receipt_entry_id']."</td>";
  echo "<td align='left' >".$rs['receipt_type']."</td>";
  echo "<td align='left' >".$rs['receipt_mode']."</td>";
	echo "<td  align='center' >".rev_convert_date($rs['receipt_date'])."</td>";
...
$disp_agent_name=array_search($rs['agent_account_code'],$company_array);
  ...
  echo "<td  align='right' >".zeroToBlank($rs['receipt_amount'])."</td>";
  echo "<td align='right' >".zeroToBlank($rs['deduction_amount'])."</td>";
