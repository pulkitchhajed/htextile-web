...
<title>Commission Add Receipt Entry</title>
	...
	if(finalCheck()) {
		...
		if((l_vou_type_selected!="Advance Receipt" )  ){
			...
			for(bn=0;bn<l_size;bn++){
				//alert(bn);
				//l_commission_bill_entry_id_array[bn].disabled=false;  - Now Hidden
		...
		if(document.getElementById('receipt_type_tag').value=="FALSE"){
			...
			document.getElementById('payment_vou').action='process_commission_receipt_entry.php?action=add';
...
function first_submit() {
	...
	if(check()) {
		document.getElementById('payment_vou').action='commission_add_receipt_entry.php';
		document.getElementById('payment_vou').submit();
		return true;
	}else{
		alert ("Validation Failed Form Not Submitted")
		return false;
	}
...
<table width="100%" border="0" align="center" style="border:0px solid #e5f1f8;background-color:#FFFFFF">
            				...
            				<table width="100%">
								<tr>								
									<td width="25%" align='left'>
										<h3>Commission Add Receipt Entry :</h3>
									</td>
	...
	<tr>
		...
		<td align="center" ><input type="text" onChange="validatedate_format(this)"  name="receipt_date" class="datepick" id="receipt_date" value="<?php echo $receipt_date;?>" size="8"></td>
		...
		<select onChange="receiptTypeChange()" name="receipt_type" id="receipt_type">
		...
		<select onChange="receiptModeChange()" name="receipt_mode" id="receipt_mode">
        		<option value="">--Select--</option>
				<?php
    ...
    <tr>
		...
		<input type="hidden" name="agent_last_value" id="agent_last_value"  value="<?php echo $agent;?>" >
        	<select name="agent_account_code" id="agent_account_code" onChange="agentChange()">
            	<option value="">--Select--</option>
                <?php
	...
	<TD align="center">
		...
		<input  type="text" size='6' onblur="recptAmountOnChange()" name="receipt_amount" id="receipt_amount"  value="<?php echo $receipt_amount;?>" >
	</TD>
	...
	<TD align="center">
		...
		<input   type="text" size='6' onblur="deductionAmountOnChange()" name="deduction_amount" id="deduction_amount"  value="<?php echo $deduction_amount; ?>" ></TD>
...
if($receipt_type!="" ){
	...
	$bill_sql="SELECT * 
				...
				ORDER BY commission_bill_date ASC, commission_bill_number ASC ";
	//echo $bill_sql;
	...
	<div class="table_scroll_h" style="height:200px; width: 1400px; border:1px solid;">		
		<table width='100%' class="tbl_border">	
			...
			<th class="th-sticky" align="left">Id - Bill No.</th>		
	...
	?>
				...
				<input type="hidden" name="commission_bill_entry_id[]" id='commission_bill_entry_id' value="<?php echo $rs['commission_bill_entry_id']; ?>">
				<?php echo $rs['commission_bill_entry_id']; ?>
				- 
				<?php echo $rs['commission_bill_number']; ?>
