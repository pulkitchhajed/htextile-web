	...
	if(($_REQUEST['manual_voucher_number']) && ($_SESSION['uid']==77)) 	{
		...
		if($_FILES['supporting_doc_1']['name']!="") { // only if files are selected -- to pevent blank entry in table
			...
			if($file_size_supporting_doc_1 > 2097152){
				...
				$upload_result = supabase_upload_file($file_tmp_supporting_doc_1, "payments/" . $file_name_supporting_doc_1, $file_type_supporting_doc_1);
				if (!$upload_result['success']) { error_log("Supabase upload failed: " . $upload_result['error']); }
				$supporting_doc_1="payments/".$file_name_supporting_doc_1;
		...
		if($_FILES['supporting_doc_2']['name']!="") { // only if files are selected -- to pevent blank entry in table
			...
			if($file_size_supporting_doc_2 > 2097152){
				...
				$upload_result = supabase_upload_file($file_tmp_supporting_doc_2, "payments/" . $file_name_supporting_doc_2, $file_type_supporting_doc_2);
				if (!$upload_result['success']) { error_log("Supabase upload failed: " . $upload_result['error']); }
				$supporting_doc_2="payments/".$file_name_supporting_doc_2;
